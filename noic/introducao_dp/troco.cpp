...
{
  if (matriz[coin][soma])
    return; // se já tivermos calculado esse estado da PD, a função retorna
  ...
  if (coin > m || soma > v)
    return; // a função retorna

  busca(coin + 1,
        soma +
            moeda[coin]); // testamos todas a possibilidades com a moeda atual
...
int main() {
  ...
  for (int i = 1; i <= m; i++)
    scanf("%d", &moeda[i]); // lemos os valores de cada moeda
